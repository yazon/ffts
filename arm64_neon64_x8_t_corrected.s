...
neon64_x8_t:
  ...
  // Optimized to match ARM32 efficiency
  mov      x11, #0              // Initialize to 0
  sub      x11, x11, x1, lsr #5 // x11 = 0 - (stride >> 5) = -(stride/32)
...
1:  // === Main Loop Body ===
  ...
  adds     x11, x11, #1                   // Increment counter (starts negative, counts to 0)
  ...
  // CORRECTED: Store intermediate results to data[1] WITHOUT incrementing pointer
  st1      {v4.4s,  v5.4s},  [x4]        // Store to data[1] (no increment)
  ...
  // CORRECTED: Store intermediate results to data[3] WITHOUT incrementing pointer
  st1      {v6.4s,  v7.4s},  [x6]        // Store to data[3] (no increment)
