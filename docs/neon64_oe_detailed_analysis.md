...
## Required fixes for `neon64_oe`

Below is a precise checklist of changes needed in `src/neon64.s` inside the `neon64_oe` routine to make the AArch64 port functionally correct and match the ARM32 `neon_oe` semantics. Line numbers refer to the current `src/neon64.s` as of this analysis.

- Initialize `q12` (v12) before first use (CRITICAL)
  - Problem: `v12` is consumed at 1421/1435 (first butterfly) before it is constructed. The intended ARM32 sequence builds `q12` from halves of `q8` and `q10` using vorr/vtrn on d-lanes.
  - Fix (insert right after the initial loads and before 1421):
    - Extract `d21` (high half of `q10`) first: `ext v21.16b, v10.16b, v10.16b, #8`.
    - Duplicate sources and build `q12` via 32-bit transposes:
      - Copy `q10` into a temp, copy `q8` into a temp.
      - `trn1 v16.4s, v24.4s, v25.4s; trn2 v25.4s, v24.4s, v25.4s; mov v24.16b, v16.16b` → yields one half of `q12`.
      - `trn1 v16.4s, v20.4s, v21.4s; trn2 v21.4s, v20.4s, v21.4s; mov v20.16b, v16.16b` → prepares the other half consistent with the ARM32 `vtrn.32 d20,d21`.
      - Finally move the assembled halves into `v12` so subsequent `fsub/fadd` against `v10` use a valid `q12`.
    - Outcome: `v12` is valid before 1421 and 1435.

- Remove the no-op pre-shuffle of `v24/v25` (lines 1407–1411) or replace it with the real pre-copy as above
  - Problem: `trn1/trn2` on `v24/v25` occurs before those registers are initialized.
  - Fix: Delete the placeholder shuffles and replace with the concrete `ext/mov/trn1/trn2` sequence described in the previous item.

- Ensure high-half extraction before any `vtrn` that conceptually uses `d21`
  - Problem: When mirroring `vtrn.32 d20,d21` from ARM32, AArch64 must explicitly materialize `d21` from `q10`.
  - Fix: Keep `ext v21.16b, v10.16b, v10.16b, #8` immediately before the `vtrn.32 d20,d21`-equivalent `trn1/trn2` pair.

- Offsets loads width and post-increment: keep 32-bit loads paired with 8-byte pointer advance
  - Context: The plan’s `offsets` are stored as 64-bit elements. The current code uses `ldr w2, [x12], #8` and `ldr w14, [x12], #8` (lines 1401, 1413), which read 32-bit values from 64-bit slots and advance by 8. This is correct given `struct _ffts_plan_t { ptrdiff_t *offsets; }` on AArch64.
  - Action: No change required to the `#8` post-increments, but keep using `w`-width loads and `uxtw #2` in address adds (see next item).

- Address calculation must use logical zero-extend scaling
  - Ensure both adds use: `add x2, x0, w2, uxtw #2` (1416) and `add x14, x0, w14, uxtw #2` (1425). This already matches ARM32 `lsl #2` semantics; keep as-is.

- Correct `vswp` emulation sites to swap only the intended 64-bit halves
  - Wherever the ARM32 code does `vswp d1,d2`, `vswp d5,d6`, `vswp d9,d10`, or `vswp d13,d14`, the AArch64 port must swap the specific 64-bit lanes, not whole vectors.
  - Pattern to use (with a temp, e.g., `v31`):
    - `orr v31.16b, vA.16b, vA.16b; mov vA.d[1], vB.d[0]; mov vB.d[0], v31.d[1]`.
  - Audit all four swap points in `neon64_oe` and ensure this pattern is used.

- Safe `trn1/trn2` usage when a destination is also a later source
  - Where a `trn1` result would overwrite a register that is still needed by an ensuing `trn2`, use a temp:
    - `trn1 v31.4s, vSRC1.4s, vSRC2.4s; trn2 vDST.4s, vSRC1.4s, vSRC2.4s; mov vSRC1.16b, v31.16b`.
  - Apply to any `q11/q9` and `q10/q8` transpose pairs before twiddle multiplies.

- Optional: gate `brk` debug markers or remove for release
  - Current `brk #0x0E..` markers at 1344/1347/1376/1403/1427 etc. are useful for bring-up but should be compiled out or flagged behind a debug macro for production builds.

- Call-site hygiene (JIT): ensure `x12` reload before OE entry
  - Outside the blob, make sure the generator resets `x12 = plan->offsets` immediately before invoking `neon64_oe` so the offsets stream starts at the correct slice.

With these fixes, `neon64_oe` aligns with the ARM32 macro’s dataflow: `q12` is valid when used, lane swaps/transposes preserve semantics, and store addresses are computed safely and correctly.
